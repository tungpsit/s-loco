...
export const vendors = pgTable(
  'vendors',
  {
    ...
    commissionRate: decimal('commission_rate', { precision: 5, scale: 2 })
      .notNull()
      .default('8.00'),
    appDiscountPercent: decimal('app_discount_percent', { precision: 5, scale: 2 })
      .notNull()
      .default('5.00'),
...
export const combos = pgTable(
  ...
  (table) => [index('combos_vendor_id_idx').on(table.vendorId)],
)
...
// ─── Vendor QR Secrets ─────────────────────────────────
export const vendorQrSecrets = pgTable(
  'vendor_qr_secrets',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    vendorId: uuid('vendor_id')
  ...
  (table) => [index('vendor_qr_secrets_vendor_id_idx').on(table.vendorId)],
)
