...
type SettlementApiItem = SettlementItem & {
  settlement?: SettlementItem
  vendor?: { name?: string }
}

/** API returns { settlement, vendor } objects — normalise */
function normalise(raw: SettlementApiItem[]): SettlementItem[] {
  return raw.map((r) => {
    if (r.settlement) {
      return {
        ...r.settlement,
        status: r.settlement.status ?? 'pending',
        vendorName: r.vendor?.name || 'Không rõ',
      }
    }
  ...
  wallet: (
    ...
    >
      <path
        ...
        d="M21 12a2.25 2.25 0 00-2.25-2.25H15a3 3 0 110-6h5.25A2.25 2.25 0 0121 6m0 6v6a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 18V6a2.25 2.25 0 012.25-2.25h13.5"
      />
    </svg>
  ),
  chart: (
...
export default function SettlementsPage() {
  ...
  const rawItems = Array.isArray(rawData) ? (rawData as unknown as SettlementApiItem[]) : []
  ...
  const disbursed = settlements.reduce(
    (s, b) => (b.status === 'disbursed' ? s + Number(b.netAmount || 0) : s),
    0,
  )
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-1">
            Duyệt và giải ngân cho vendor · hoa hồng 8%
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-outline-variant/10 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-on-surface-variant">
            ...
            <p className="text-sm">
              Nhấn &quot;Chạy batch mới&quot; để tạo đợt thanh toán đầu tiên.
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-outline-variant/10 bg-surface-low/40">
                    <th className="text-left px-5 py-3.5 font-semibold text-on-surface-variant text-xs uppercase tracking-wider">
                      Mã
                    </th>
                    <th className="text-left px-5 py-3.5 font-semibold text-on-surface-variant text-xs uppercase tracking-wider">
                      Vendor
                    ...
                    <th className="text-right px-5 py-3.5 font-semibold text-on-surface-variant text-xs uppercase tracking-wider">
                      Tổng
                    </th>
                    <th className="text-right px-5 py-3.5 font-semibold text-on-surface-variant text-xs uppercase tracking-wider">
                      Hoa hồng
                    ...
                    <th className="text-center px-5 py-3.5 font-semibold text-on-surface-variant text-xs uppercase tracking-wider">
                      Voucher
                    </th>
                    <th className="text-center px-5 py-3.5 font-semibold text-on-surface-variant text-xs uppercase tracking-wider">
                      Trạng thái
                ...
                <tbody className="divide-y divide-outline-variant/5">
                  {settlements.map((s) => (
                    <tr key={s.id} className="hover:bg-primary/[0.03] transition-colors">
                      ...
                      <td className="px-5 py-4 text-right font-medium text-on-surface whitespace-nowrap">
                        {fmt(s.totalAmount)}
                      </td>
                      <td className="px-5 py-4 text-right text-error whitespace-nowrap">
                ...
                <div key={s.id} className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3 rounded-xl bg-surface-low/50 p-3">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs font-bold text-error mt-0.5">
                        {fmt(s.commissionAmount)}
                      </p>
                    </div>
                    <div className="text-center">
...
function Badge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-semibold ring-1 whitespace-nowrap ${st.cls}`}
    >
      {st.label}
    </span>
  )
...
function Actions({
  ...
  disburse: { isPending: boolean; mutate: (id: string) => void }
}) {
    ...
    <div className="flex items-center justify-end gap-2">
      {s.status === 'pending' && (
        <>
          <button
            type="button"
            onClick={() => s.id && approve.mutate(s.id)}
            disabled={busy}
            className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-primary text-white hover:bg-primary/90 disabled:opacity-40 transition-colors"
          >
            Duyệt
          </button>
          <button
            type="button"
            onClick={() => s.id && reject.mutate(s.id)}
            disabled={busy}
            className="px-3.5 py-1.5 text-xs font-semibold rounded-lg text-error bg-error/5 hover:bg-error/10 disabled:opacity-40 transition-colors"
          >
            Từ chối
          </button>
        </>
      ...
      {s.status === 'approved' && (
        <button
          type="button"
          onClick={() => s.id && disburse.mutate(s.id)}
          disabled={busy}
          className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40 transition-colors"
        >
          Giải ngân
        </button>
      )}
