...
function CreateArticleDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setTitle(''); setSlug(''); setCategory(''); setContent(''); setCoverImage(''); setIsPublished(false)
      setErrors({})
      onClose()
    },
    onError: (err: any) => {
      setErrors({ form: err.message || 'Lỗi khi tạo bài viết' })
    },
  })
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center'>
      ...
      <div className='relative z-50 w-full max-w-lg mx-4 bg-white rounded-2xl shadow-xl overflow-y-auto max-h-[90vh]'>
        ...
        <div className='flex items-center justify-between px-6 py-4 border-b border-outline-variant/15'>
          ...
          <button onClick={onClose} className='flex h-8 w-8 items-center justify-center rounded-lg text-on-surface-variant hover:bg-surface-high'>
        ...
        <form onSubmit={handleSubmit} className='p-6 space-y-4'>
          ...
          <div className='flex items-center gap-2'>
            ...
            <label className='text-sm text-on-surface cursor-pointer' onClick={() => setIsPublished(!isPublished)}>Xuất bản ngay</label>
          </div>
          ...
          <div className='flex justify-end gap-3 pt-2'>
            ...
            >
              {createMut.isPending ? 'Đang tạo…' : 'Tạo bài viết'}
            </button>
          </div>
        </form>
      </div>
