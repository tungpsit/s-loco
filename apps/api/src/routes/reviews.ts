...
reviewRoutes.get('/:vendorId', async (c) => {
  ...
  const page = Number(c.req.query('page') || 1); const limit = Number(c.req.query('limit') || 20)
...
reviewRoutes.post('/', authMiddleware(), async (c) => {
  try {
    ...
    const body = await c.req.json() as {
      vendor_id: string; service_id?: string; voucher_id?: string; rating: number; comment?: string
    }
    const result = await reviewSvc.createReview(userId, {
      vendorId: body.vendor_id, serviceId: body.service_id, voucherId: body.voucher_id,
      rating: body.rating, comment: body.comment,
    })
    return c.json({ success: true, data: result }, 201)
  } catch (err) {
    if (err instanceof ReviewError) {
      return c.json({ success: false, error: { code: err.code, message: err.message } }, err.code === 'DUPLICATE' ? 409 : 400)
    }
    throw err
  }
})
