...
import { initiatePaymentSchema, refundRequestSchema } from '@s-local/shared/validators'
...
paymentRoutes.post(
  ...
  async (c) => {
    try {
      ...
      const result = await paymentSvc.initiatePayment(order_id, userId, gateway, ipAddress || undefined)
...
paymentRoutes.get(
  ...
  async (c) => {
    ...
    return c.json({ success: true, data: { payment_status: status, message: status === 'success' ? 'Thanh toán thành công!' : 'Thanh toán thất bại.' } })
  },
)
