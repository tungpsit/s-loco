...
export async function getAdminDashboard() {
  ...
  return {
    ...
    settlements: {
      totalDisbursed: settlStats!.totalDisbursed,
      totalCommission: settlStats!.totalCommission,
      pendingCount: Number(settlStats!.pending),
    },
  }
}
...
export async function listAllOrders(opts: {
  ...
  const rows = await db.select({ count: sql<number>`count(*)` }).from(orders).where(where)
