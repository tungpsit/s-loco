...
interface ComboItem { serviceId: string; quantity: number }

export async function createCombo(vendorId: string, data: {
  name: string; description?: string; items: ComboItem[]; comboPrice: number; images?: string[]
}) {
  const db = getDb()
  for (const item of data.items) {
    const [svc] = await db.select().from(services)
      .where(and(eq(services.id, item.serviceId), eq(services.vendorId, vendorId))).limit(1)
  ...
  const [combo] = await db.insert(services).values({
    vendorId, name: data.name, description: data.description,
    originalPrice: String(originalTotal), discountPrice: String(data.comboPrice),
    images: data.images || [], comboItems: data.items, isActive: true, maxQuantityPerOrder: 5,
  } as any).returning()
  return combo
}
...
export async function listCombos(opts: { vendor_id?: string; page?: number; limit?: number }) {
  ...
  const conditions = [sql`${services.comboItems} IS NOT NULL`, eq(services.isActive, true)]
  if (opts.vendor_id) conditions.push(eq(services.vendorId, opts.vendor_id))
  const items = await db.select({ combo: services, vendor: { id: vendors.id, name: vendors.name } })
    .from(services).innerJoin(vendors, eq(services.vendorId, vendors.id))
    .where(and(...conditions)).limit(limit).offset(offset)
  const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(services).where(and(...conditions))
...
export async function purchaseCombo(comboServiceId: string, userId: string, quantity: number) {
  ...
  const [combo] = await db.select().from(services)
    .where(and(eq(services.id, comboServiceId), sql`${services.comboItems} IS NOT NULL`)).limit(1)
  ...
  const result = await db.transaction(async (tx) => {
    const [order] = await tx.insert(orders).values({
      userId, totalAmount: String(Number(combo.originalPrice) * quantity),
      discountAmount: String(Number(combo.originalPrice) * quantity - finalAmount),
      finalAmount: String(finalAmount), note: `Combo: ${combo.name}`,
    }).returning()
    ...
    for (let q = 0; q < quantity; q++) {
      for (const item of comboItems) {
        ...
        const [orderItem] = await tx.insert(orderItems).values({
          orderId: order!.id, serviceId: item.serviceId, vendorId: svc.vendorId,
          quantity: item.quantity, unitPrice: svc.discountPrice || svc.originalPrice,
        ...
        for (let i = 0; i < item.quantity; i++) {
          const [voucher] = await tx.insert(vouchers).values({
            orderItemId: orderItem!.id, userId, vendorId: svc.vendorId,
            serviceId: item.serviceId, code: generateVoucherCode(), status: 'created', expiresAt,
          }).returning()
...
export class ComboError extends Error {
  code: string
  constructor(code: string, message: string) { super(message); this.code = code; this.name = 'ComboError' }
}
