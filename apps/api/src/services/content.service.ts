...
export async function listArticles(opts: { category?: string; page?: number; limit?: number }) {
  ...
  const items = await db.select().from(articles).where(and(...conditions))
    .orderBy(sql`${articles.publishedAt} DESC NULLS LAST`).limit(limit).offset(offset)
  const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(articles).where(and(...conditions))
...
export async function getArticleBySlug(slug: string) {
  ...
  const [article] = await db.select().from(articles)
    .where(and(eq(articles.slug, slug), eq(articles.isPublished, true))).limit(1)
...
export async function createArticle(authorId: string, data: {
  ...
  const [article] = await db.insert(articles).values({
    ...data, authorId, publishedAt: data.isPublished ? new Date() : null,
  }).returning()
  return article!
}
...
export async function getWeather() {
  if (weatherCache && Date.now() - weatherCache.cachedAt < WEATHER_CACHE_TTL) return weatherCache.data
  try {
    ...
    return data
  } catch { return { error: 'Không thể lấy dữ liệu thời tiết.' } }
}
...
export async function listUpcomingEvents(limit = 10) {
  ...
  return db.select().from(articles)
    ...
    .orderBy(sql`${articles.publishedAt} DESC`).limit(limit)
}

export class ContentError extends Error {
  code: string
  constructor(code: string, message: string) { super(message); this.code = code; this.name = 'ContentError' }
}
