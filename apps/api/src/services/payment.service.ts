...
import { orderItems, orders, payments, vouchers } from '@s-local/db/schema'
...
// ─── Initiate payment ──────────────────────────────────
export async function initiatePayment(orderId: string, userId: string, gateway: string, ipAddress?: string) {
  const db = getDb()
...
export async function processWebhook(gateway: string, payload: Record<string, unknown>, signature: string) {
  const gw = gateways[gateway]
...
export async function pollPendingPayments() {
  ...
  const stale = await db.select().from(payments)
