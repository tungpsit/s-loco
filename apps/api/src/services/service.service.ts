...
import type { CreateServiceInput, UpdateServiceInput } from '@s-local/shared/validators'
...
export async function createService(vendorId: string, ownerId: string, data: CreateServiceInput) {
  ...
  const [vendor] = await db.select().from(vendors)
    ...
    .limit(1)
  if (!vendor) throw new ServiceError('FORBIDDEN', 'Bạn không có quyền thêm dịch vụ cho cửa hàng này.')
...
export async function updateService(serviceId: string, ownerId: string, data: UpdateServiceInput) {
  ...
  const [svc] = await db.select({ service: services, vendor: vendors })
    .from(services)
  ...
  if (data.max_quantity_per_order !== undefined) updateData.maxQuantityPerOrder = data.max_quantity_per_order
...
export async function getServiceById(serviceId: string) {
  const db = getDb()
  const [svc] = await db.select({ service: services, vendor: vendors, category: serviceCategories })
    .from(services)
    .innerJoin(vendors, eq(services.vendorId, vendors.id))
