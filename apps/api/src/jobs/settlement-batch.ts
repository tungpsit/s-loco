...
export async function runSettlementBatch(periodDays = 3) {
  ...
  const vendorRows = await db.selectDistinct({ vendorId: vouchers.vendorId })
    .from(vouchers)
    .where(and(
  ...
  console.log(`[SettlementBatch] Created ${results.length} settlement(s) for period ${periodStart.toISOString()} → ${periodEnd.toISOString()}`)
